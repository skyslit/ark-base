...
export default createComponent((props) => {
    ...
    const adminValidation = (data: any) => {
        adminValidationService.invoke(
            ...
            { force: true })
            .then((res) => {
                { res.data.length === 0 ? history.push("/create/admin") : history.push(`/auth/login${history.location.search}`) }
            })
            .catch(() => {
            })
    }
                ...
                <Row className="main-wrapper">
                    <Col xs={24} sm={24} md={24} lg={12} xl={12} className="main-login">
                        ...
                        <div className="main-div">
                            <div>
                                <Form className="main-form" layout="vertical" onFinish={_login} name="email">
                                    ...
                                    <div style={{ display: "flex", justifyContent: "end" }}>
                                        <Link className="forgot-pass-btn" to="/account/recovery">Forgot password?</Link>
                                    </div>
                                    <Form.Item>
                                        <Button type="primary" htmlType="submit" disabled={loginService.isLoading}>
                            ...
                            <div className="dont-text" style={{ flexDirection: "row", display: "flex" }}>
                                <Typography  >
                                    Don’t have an account?
                                </Typography>
                                <Link to="/auth/account/register" className="register-link">Register</Link>
                            </div>
                        </div>
                    </Col>
